            ...
            if (realSeatInformation[p]!!.keys.size > 1) {
                connection.createStatement().use {
                    for(seatNo in realSeatInformation[p]!![1]!!){
                        val query = "UPDATE seats SET extra_space=true WHERE aircraft_code = '$planeCode' " +
...
fun getAvailablePlanesAndConditions(statement: Statement)
    ...
    val sqlRequest = "SELECT aircraft_code, fare_conditions FROM seats " +
...
fun getFlights(statement: Statement): List<Flight> {
    val sql = "SELECT flight_id, aircraft_code FROM flights ORDER BY flight_id"
